...
int checkUniqueSud3() {
  for (int i = 0; i<82; i++) {
    ...
    } else if (sallbool[i]==false&& i==81) //Wanneer het laatste vakje word berijkt word er een oplossing aan toe gekend.
    {                                      //Hierna word er weer een stapje terug gegaan om zo nog een mogelijke 
      totaleoplossingen +=1;               //oplossing te vinden.
      for (int j = 1; j<81; j++) {
        if (sallbool[i-j]==true) {   
          i= i-j-1;
          j=82;
    ...
    } else if (totaleoplossingen>1) //Als er meer dan 1 oplossing is is de sudoku niet uniek en moet alles opnieuw.
    {                               //Elke waarde word gereset en de g word op 0 gezet.
      i=82;
      bevatweluny = 0;
      bevatnietuny = 0;
      totaleoplossingen = 0;
  ...
  //als er maar 1 oplossing en elke mogelijkheid is doorlopen dan word de sudoku goed gekeurd.
   if (totaleoplossingen ==1) {
    for (int i = 1; i<82; i++) { //Elke waarde die niet in de sudoku hoort word verwijderd.
    ...
    totaleoplossingen = 0;
    println("Gevonden!");
    int g =2; //Deze g=2 zorgt ervoor de dat while loop word onderbroken en de sudoku is klaar.
    return g;
  }
  bevatweluny = 0;
  bevatnietuny = 0;
